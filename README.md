...
1. PHP + Laravel
2. Javascript ou Typescript + NodeJS + Express

## Requisitos

1. O sistema deve ter um mecanismo de login usando JWT, com um entrypoint que recebe `{ "login":"clickideia", "senha":"clk@123"}` e gera um token.
...
3. O login e senha fornecidos devem estar em variáveis de ambiente e terem uma versão para o ambiente de desenvolvimento vinda de um arquivo .env. Esse arquivo não deve subir ao GIT, mas sim um arquivo de exemplo sem os valores reais. O mesmo vale para qualquer "segredo" do sistema, como a chave do JWT.
...
10. Deve ser usada alguma forma de persistência dos dados, mesmo que em memória.
...
13. O projeto deve ser colocado em um repositório GITHUB ou equivalente, estar público, e conter um readme.md que explique em detalhes qualquer comando ou configuração necessária para fazer o projeto rodar. Por exemplo, como configurar as variáveis de ambiente, como rodar migrations (se foram usadas). 

14. A entrega será apenas a URL para clonarmos o repositório.
...
{ "login":"clickideia", "senha":"clk@123"}
```
...
15. Priorize componentes funcionais e hooks.
